...
Runs a demo-style shell script generated from demos/flask-redis/README.md.
...
pe "$(cat <<'EOF'
  sed -i.bak "s|image: redis:7-bookworm-scone|image: ${IMAGE_NAME}-redis-scone|g" "$DEMO_DIR/manifests/manifest.prod.sanitized.yaml"
EOF
)"
pe "$(cat <<'EOF'
...
printf "%b" "$LILAC"
printf '%s\n' ''
printf '%s\n' '`push-image: true` in the templates pushes the confidential images automatically.'
printf '%s\n' ''
printf '%s\n' '---'
printf '%s\n' ''
