...
printf '%s\n' '`TEE_TYPE` are available to derive the CAS session namespace below. Default values live in `$DEMO_DIR/values.template.yaml`; copy it to `Values.yaml` if that file does not already exist:'
printf '%s\n' ''
printf "%b" "$RESET"
...
pe "$(cat <<'EOF'
(cd "$DEMO_DIR" && scone-td-build apply -f manifests/scone.v1.yaml)
EOF
)"
...
pe "$(cat <<'EOF'
(cd "$DEMO_DIR" && scone-td-build apply -f manifests/scone.v2.yaml)
EOF
)"
