...
printf '%s\n' '- `IMAGE_NAME` set in `Values.yaml` to an image path in a registry you can push to.'
printf '%s\n' ''
...
pe "$(cat <<'EOF'
export SCONE_KEY="$DEMO_DIR/manifests/identity.pem"
EOF
)"
...
pe "$(cat <<'EOF'
docker build --build-arg "PETCLINIC_REF=$PETCLINIC_REF" -t "$IMAGE_NAME" "$DEMO_DIR"
EOF
)"
pe "$(cat <<'EOF'
...
pe "$(cat <<'EOF'
(cd "$DEMO_DIR" && scone-td-build apply -f manifests/scone.yaml)
EOF
)"
...
printf '%s\n' ''
printf '%s\n' '- `reference-image: busybox` (the default skip-list). The reference image is a skip-list: any binary present in it is left untouched by the sconify step. `java` is dynamically linked against glibc and dlopens the JRE `.so` libraries, so its whole dependency tree must be sconified into the enclave link map. Pointing the skip-list at the JRE base (or the target image itself) would drop those dependencies and the enclave would abort with `libpthread ... not found in the enclave link map`.'
