This file defines the environment variables used to configure the `go-args-env-file` example. The variables below are collected with `tplenv`:

1. The native `go-args-env-file` application uses a container image.
