...
Run both versions natively first to confirm the application works before adding SCONE protection. The native run uses a placeholder `API_PASSWORD` stored in a Kubernetes `Secret`.

### Step 6. Deploy and verify native Version 1

```bash
# Create a placeholder Kubernetes Secret for the native run.
kubectl create secret generic api-credentials --namespace ${NAMESPACE} --from-literal=API_PASSWORD="native-test-password" --dry-run=client -o yaml | kubectl apply -f -
